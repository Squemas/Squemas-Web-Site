...
class usersModel extends IdEnModel
	{
		...
		public function getUserNameExists($vUserName)
			{
				...
				$vResultUserExists = $this->vDataBase->query("SELECT
                                                                COUNT(*)
                                                            FROM tb_squemas_users
                                                            WHERE tb_squemas_users.c_username = '$vUserName';");
		...
		public function getUserEmailExists($vUserEmail)
			{
				...
				$vResultUserEmailExists = $this->vDataBase->query("SELECT
                                                                COUNT(*)
                                                            FROM tb_squemas_users
                                                                WHERE tb_squemas_users.c_email = '$vUserEmail';");
		...
		public function getUserName($vUserCode)
			{
				...
				$vResultUserName = $this->vDataBase->query("SELECT
                                                            ...
                                                            FROM tb_squemas_users
                                                                WHERE tb_squemas_users.n_coduser = $vUserCode;");
		...
		public function getUserCodeFromEmailActivationCode($vActivationCode, $vEmail)
			{
				...
				$vResultUserCodeFromEmailActivationCode = $this->vDataBase->query("SELECT
                                                            ...
                                                            FROM tb_squemas_users
                                                                WHERE tb_squemas_users.n_activationcode = $vActivationCode
		...
		public function getUserActivationCode($vEmail)
			{
				...
				$vResultUserActivationCode = $this->vDataBase->query("SELECT
                                                                tb_squemas_users.n_activationcode
                                                            FROM tb_squemas_users
                                                                WHERE tb_squemas_users.c_email = '$vEmail';");
		...
		public function getUser($vUserCode)
			{
				...
				$vResult = $this->vDataBase->query("SELECT
                                                        ...
                                                        tb_squemas_users.n_activationcode,
                                                        tb_squemas_users.n_active,
                                                        tb_squemas_users.c_usercreate,
                                                        tb_squemas_users.d_datecreate,
                                                        tb_squemas_users.c_usermod,
                                                        tb_squemas_users.d_datemod
                                                    FROM tb_squemas_users
                                                        WHERE tb_squemas_users.n_coduser = $vUserCode;");
		...
		public function getUserInfo($vUserCode)
			{
				...
				$vResult = $this->vDataBase->query("SELECT
                                                        ...
                                                        tb_squemas_usernames.n_coduser,
                                                        tb_squemas_usernames.c_names,
                                                        tb_squemas_usernames.c_lastnames,
                                                        tb_squemas_usernames.c_othername,
                                                        tb_squemas_usernames.d_birthdate,
                                                        tb_squemas_usernames.c_country,
                                                        tb_squemas_usernames.c_city,
                                                        tb_squemas_usernames.n_active,
                                                        tb_squemas_usernames.c_usercreate,
                                                        tb_squemas_usernames.d_datecreate,
                                                        tb_squemas_usernames.c_usermod,
                                                        tb_squemas_usernames.d_datemod
                                                    FROM tb_squemas_usernames
                                                        WHERE tb_squemas_usernames.n_coduser = $vUserCode;");
				return $vResult->fetchAll();
				$vResult->close();
			}
		...
		public function getUserNamesComplete($vUserCode)
			{
				...
				$vResult = $this->vDataBase->query("SELECT
                                                        CONCAT(tb_squemas_usernames.c_names,' ',tb_squemas_usernames.c_lastnames) AS c_namescomplete
                                                    FROM tb_squemas_usernames
                                                        WHERE tb_squemas_usernames.n_coduser = $vUserCode;");
		...
		public function userRegister($vUsername, $vPassword1, $vPassword2, $vEmail, $vRole, $vActivationcode, $vActive){
				...
				$vResultUserRegister = $this->vDataBase->prepare("INSERT INTO tb_squemas_users(c_username, c_pass1, c_pass2, c_email, c_userrole, n_activationcode, n_active, c_usercreate, d_datecreate)
		...
		public function userInfoRegister($vUserCode, $vNames, $vLastNames, $vOthername, $vBirthDate, $vCountry, $vCity, $vActive){
				...
				$vResultUserInfoRegister = $this->vDataBase->prepare("INSERT INTO tb_squemas_usernames(n_coduser, c_names, c_lastnames, c_othername, d_birthdate, c_country, c_city, n_active, c_usercreate, d_datecreate)
		...
		public function updateUserStatus($vUserCode, $vActive){
                ...
                $vResultUpdateUserStatus = $this->vDataBase->prepare("UPDATE
                                                tb_squemas_users
                                            SET tb_squemas_users.n_active = :n_active,
                                                tb_squemas_users.c_usermod = :c_usermod,
                                                tb_squemas_users.d_datemod = NOW()
                                            WHERE tb_squemas_users.n_coduser = :n_coduser;")
                                ->execute(
                                            array(
                                                    ':n_active'=>$vActive,
                                                    ':c_usermod'=>$vUserMod,
		...
		public function updateUserInfoStatus($vUserCode, $vActive){
                ...
                $vResultUpdateUserInfoStatus = $this->vDataBase->prepare("UPDATE
                                                tb_squemas_usernames
                                            SET tb_squemas_usernames.n_active = :n_active,
                                                tb_squemas_usernames.c_usermod = :c_usermod,
                                                tb_squemas_usernames.d_datemod = NOW()
                                            WHERE tb_squemas_usernames.n_coduser = :n_coduser;")
                                ->execute(
                                            array(
                                                    ':n_active'=>$vActive,
                                                    ':c_usermod'=>$vUserMod,
